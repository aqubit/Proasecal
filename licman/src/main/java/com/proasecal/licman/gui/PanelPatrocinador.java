...
import com.proasecal.licman.controller.PatrocinadorGridController;

public class PanelPatrocinador extends JPanel {
	...
	private JTextField txtFldNombrePatrocinador;
	 PatrocinadorGridController controller;
	...
	public PanelPatrocinador() {
		...
		JLabel lblNombrePatrocinador = new JLabel(
				BUNDLE.getString("PanelPatrocinador.lblNombrePatrocinador.text")); //$NON-NLS-1$
		pnlGrupo1.add(lblNombrePatrocinador, "2, 2, default, top");

		txtFldNombrePatrocinador = new JTextField();
		txtFldNombrePatrocinador.setText("");
		txtFldNombrePatrocinador.setColumns(20);
		pnlGrupo1.add(txtFldNombrePatrocinador, "2, 4, left, default");
		...
		JLabel lblResultados = new JLabel(
				BUNDLE.getString("PanelPatrocinador.lblResultados.text")); //$NON-NLS-1$
		...
		controller = new PatrocinadorGridController(gridControl);
		...
		gridControl
				.setValueObjectClassName(BUNDLE
						.getString("PanelPatrocinador.gridControl.valueObjectClassName"));
		...
		columnNombre.setHeaderColumnName(BUNDLE
				.getString("PanelPatrocinador.columnNombre.headerColumnName")); //$NON-NLS-1$
		...
		columnVersion.setColumnName("descripcion");
		columnVersion.setHeaderColumnName(BUNDLE
				.getString("PanelPatrocinador.columnVersion.headerColumnName")); //$NON-NLS-1$
		...
		columnActivo.setHeaderColumnName(BUNDLE
				.getString("PanelPatrocinador.columnActivo.headerColumnName")); //$NON-NLS-1$
		...
		columnFechaCreacion
				.setHeaderColumnName(BUNDLE
						.getString("PanelPatrocinador.columnFechaCreacion.headerColumnName")); //$NON-NLS-1$
		gridControl.getColumnContainer().add(columnFechaCreacion);

		JButton btnBuscar = new JButton(
				BUNDLE.getString("PanelPatrocinador.btnBuscar.text")); //$NON-NLS-1$
		btnBuscar.setIcon(new ImageIcon(PanelPatrocinador.class
		...
		addComponentListener(new ComponentAdapter() {
			public void componentShown(ComponentEvent e) {
				txtFldNombrePatrocinador.requestFocusInWindow();
		...
		AbstractAction actionBuscar = new AbstractAction() {
			public void actionPerformed(ActionEvent e) {
				...
				filtro.put("nombre", txtFldNombrePatrocinador.getText());
				controller.buscar(filtro);

			}
		};
		txtFldNombrePatrocinador.getInputMap().put(
				KeyStroke.getKeyStroke(KeyEvent.VK_ENTER, 0), "Buscar");
		txtFldNombrePatrocinador.getActionMap().put("Buscar", actionBuscar);
